...
/**
 * Piece-Square Tables (PSQT) pro hodnocení pozic šachových figur.
 * Metoda getPieceTableValue vrací hodnotu z tabulky pro bílou nebo černou
 * figuru.
 * Černé figury používají invertovanou tabulku (řádek 0 = 8. řada).
 * 
 * @author mecova
 */
...
public class PSQT {
    ...
    public int getPieceTableValue(Types type, int col, int row, boolean isWhite) {
        ...
        switch (type) {
            ...
            case KNIGHT:
                return KNIGHT_TABLE[col][row];
            case BISHOP:
                return BISHOP_TABLE[col][row];
            case ROOK:
                return ROOK_TABLE[col][row];
            case QUEEN:
